...
String sql ="select * from productsjee where id="+id;
resultSet = statement.executeQuery(sql);
while(resultSet.next()){
	out.println(resultSet.getString("designation"));
	%>
...
<div class="body-content">
  <div class="module">
    <h1>Update product</h1>
		<form action="ServletP" method="get">
      <div class="alert alert-error"></div>
      <label>Product ID: </label><input readonly type="text" name="id" value="<%=resultSet.getString("id") %>" > </br></br>
      <label> Désignation:</label><input type="text" placeholder="Designation" name= "designation" value="<%=resultSet.getString("designation") %>" required />
      <label>Categorie:</label>	
		<select name="categorie" value="<%=resultSet.getString("categorie") %>">
		<option>Informatique</option>
		<option>Culture</option>
		<option>Habillement</option>
		</select>
      <label>Description</label> <textarea style ="height:200px"type ="text" placeholder="Drop your Description Here..." name="description"><%=resultSet.getString("description") %></textarea>
      
      <label>Prix unitaire:</label> <input type ="text" name= "prixU" value="<%=resultSet.getString("prix") %>">
      <label>Quantité:</label> <input type ="text" name= "quantite" value="<%=resultSet.getString("quantite") %>"> </br></br>
      <label for="Image">Add Image</label>
    	<input type="file" name="image" value="<%=resultSet.getString("image") %>"></br></br>
		<input type="submit" name="btnAdd" value="Update" class="btn btn-block btn-primary">
